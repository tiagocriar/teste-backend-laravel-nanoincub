    ...
    <div class="flex flex-wrap mt-4">
        <div class="w-full max-w-full px-3 shrink-0 md:w-6/12 md:flex-0">
            <div class="relative flex flex-wrap items-stretch w-full transition-all rounded-lg ease">
                ...
                <input type="text" name="q_nome" value="{{ $filter_data['q_nome'] ?? null }}" class="pl-9 focus:shadow-primary-outline dark:bg-slate-850 dark:text-white text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-blue-500 focus:outline-none" placeholder="Nome" />
            </div>
        </div>

        @inject('carbon', 'Carbon\Carbon')

        <div id="date-range" class="w-full max-w-full px-3 shrink-0 md:w-6/12 md:flex-0 flex flex-row gap-4">
