                        ...
                        <div class="mb-4">
                            ...
                            <div class="flex justify-end items-center relative">
                                <input type="password" name="senha" placeholder="Senha"
                                    {{ (isset($action) && $action === 'update') ? null : 'required' }} pattern=".{8,}" title="Mínimo de 8 caracteres"
                                    class="focus:shadow-primary-outline dark:bg-slate-850 dark:text-white text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-blue-500 focus:outline-none"
                                >
                                <i onclick="changePasswordType(this)" class="absolute mr-2 px-2 cursor-pointer far fa-eye"></i>
                            </div>
                            @error('senha')
                                <div class="text-red-500 text-sm">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="w-full max-w-full px-3 shrink-0 md:w-full md:flex-0">
                        <div class="mb-4">
                            ...
                            <input type="text" name="nome_completo" placeholder="Nome Completo"
                                required
                                value="{{ $funcionario->nome_completo ?? null }}"
                                class="focus:shadow-primary-outline dark:bg-slate-850 dark:text-white text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-blue-500 focus:outline-none"
                            >
                            @error('nome_completo')
                                <div class="text-red-500 text-sm">{{ $message }}</div>
